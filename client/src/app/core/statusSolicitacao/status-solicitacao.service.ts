...
export class StatusSolicitacaoService {
    ...
    list(max?: any, offset?: any): Observable<StatusSolicitacao[]> {
        let subject = new Subject<StatusSolicitacao[]>();
        this.http.get(this.baseUrl + `statusSolicitacao?offset=${offset}&max=${max}`, {headers: this.headers})
            .map((r: Response) => r)
            .subscribe((json: any) => {
                subject.next(json['statusSolicitacao'].map((statusSolicitacao: any) => new StatusSolicitacao(statusSolicitacao)))
            });
    ...
    search(searchTerm, offset?: any, limit?): Observable<any[]> {
        ...
        const url = this.baseUrl + 'statusSolicitacao';
        let subject = new Subject<StatusSolicitacao[]>();
